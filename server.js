...
server.use(bodyParser.urlencoded());        // 支援解析 URL 編碼的請求體

// 資料庫設定
var DB = require("nedb-promises");          // 引入 NeDB 資料庫的 Promise 版本

// 建立三個資料庫實例，分別用於存儲不同類型的資料
// 資料庫檔案會自動建立在指定路徑
var DishesListDB = DB.create(__dirname + "/DishesList.db");    // 餐點列表資料庫
var FoodstuffDB = DB.create(__dirname + "/Foodstuff.db");        // 食材資料庫
var CultureListDB = DB.create(__dirname + "/CultureList.db");     // 文化資料庫

var SushiTypesDB = DB.create(__dirname + "/SushiTypes.db");    // 壽司種類資料庫
var IngredientsDB = DB.create(__dirname + "/Ingredients.db");   // 食材資料庫
var TechniquesDB = DB.create(__dirname + "/Techniques.db");     // 技法資料庫
var CultureDB = DB.create(__dirname + "/Culture.db");           // 文化資料庫
// DishesListDB.insert([
